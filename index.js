...
client.on(Events.MessageCreate, async message => {
	if (message.author.bot || message.stickers.size === 0 || !message.guild) return;

	const sticker = await message.stickers.first().fetch();

	const serverTable = db.table(`stickers_${message.guildId}`);
	...
	if (!sticker.guildId) {
		...
		await serverTable.set(`${sticker.id}.guild`, sticker.guildId);
	}
});
